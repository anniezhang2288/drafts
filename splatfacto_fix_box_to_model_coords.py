...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        def foobar(button: ViewerButton):
            ...
            def handle_dragbox(dragbox: ViewerDragbox):
                # Add the current dragbox to the list
                dragboxes.append(dragbox)

                # Check if two dragboxes have been selected
                if len(dragboxes) == 2:
                    # Calculate the intersection region
                    intersection_min = np.maximum(dragboxes[0].box_min, dragboxes[1].box_min)
                    intersection_max = np.minimum(dragboxes[0].box_max, dragboxes[1].box_max)

                    # Convert screen coordinates to model coordinates, if necessary
                    # You might need to adjust this part based on your coordinate system
                    intersection_min = convert_screen_to_model_coordinates(intersection_min)
                    intersection_max = convert_screen_to_model_coordinates(intersection_max)

                    # Identify Gaussians within the intersection region
                    mask = (
                        (self.xys[:, 0] > intersection_min[0]) &
                        (self.xys[:, 0] < intersection_max[0]) &
                        (self.xys[:, 1] > intersection_min[1]) &
                        (self.xys[:, 1] < intersection_max[1])
                    )

                    # Remove Gaussians in the intersection
                    ...
                    # Update optimizer if necessary
                    self.remove_from_all_optim(self.optimizers, mask)

                    # Reset the dragboxes list for future operations
                    dragboxes.clear()

                    self.button.set_disabled(False)
                    self.viewer_control.unregister_pointer_cb(handle_dragbox)

            self.button.set_disabled(True)
            self.viewer_control.register_dragbox_cb(handle_dragbox)
