...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        def delete_cropbox(button: ViewerButton):
            def b(click: ViewerDragbox):
                print("hi")  # Use standard quotes
                # Re-enable the button and unregister the dragbox callback
                with torch.no_grad():
                    self.features_dc[0:,:] = RGB2SH(torch.tensor([1, 0, 0], device = self.device))

                self.button2.set_disabled(False)
                self.viewer_control.unregister_pointer_cb(b)

            # Disable the button and register the callback for dragbox events
            self.button2.set_disabled(True)
            self.viewer_control.register_dragbox_cb(b)

        # Create the button with the modified callback
        self.button2 = ViewerButton("delete_cropbox", cb_hook=delete_cropbox)  # Fixed typo in button name
        
        def select_gaussians(button: ViewerButton):
            def baz(click: ViewerDragbox):
                # Existing code to set up the camera and project Gaussians...
                ...
                # Keep Gaussians outside the dragbox by inverting the mask
                ...
                with torch.no_grad():
                    self.features_dc[mask, :] = RGB2SH(torch.tensor([1, 0, 0], device=self.device))

                
                self.xys = self.xys[~mask]
                self.radii = self.radii[~mask]

                # If training, retain gradients for the updated xys tensor
                if self.training:
                    self.xys.retain_grad()

                # If using visualization counts or gradients, update them for the remaining Gaussians
                ...
                # Update the optimizers for the remaining Gaussians
