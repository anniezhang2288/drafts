...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        def handle_dragboxes(dragboxes):
            masks = []
            for dragbox in dragboxes:
                box_min, box_max = np.array(dragbox.box_min), np.array(dragbox.box_max)
                box_min[1], box_max[1] = -box_max[1], -box_min[1]  # Adjust y-axis if necessary
                
                # Transform to canvas pixel coordinates, this is just an example, adjust as needed
                box_min = box_min * [self.canvas_width, self.canvas_height] + [self.canvas_width / 2, self.canvas_height / 2]
                box_max = box_max * [self.canvas_width, self.canvas_height] + [self.canvas_width / 2, self.canvas_height / 2]

                # Create mask for gaussians within the dragbox
                mask = (
            ...
            # Combine masks to find intersection
            if masks:
                combined_mask = torch.stack(masks).all(dim=0)

                # Update model parameters by removing gaussians within the intersection
                self.means = torch.nn.Parameter(self.means[~combined_mask])
                # Repeat the removal for other parameters like self.scales, self.quats, etc.

        # Initialize a list to collect dragboxes
        dragboxes = []

        # Define the callback function to be called when a dragbox is created
        def on_dragbox_created(dragbox: ViewerDragbox):
            dragboxes.append(dragbox)

            # Process the dragboxes once we have collected three of them
            if len(dragboxes) == 3:  # Adjust the number of dragboxes as required
                handle_dragboxes(dragboxes)
                dragboxes.clear()  # Clear the list for future dragbox operations

        # Create a viewer button that triggers the dragbox creation process
        self.button = ViewerButton("Process Dragboxes", cb_hook=on_dragbox_created)
        self.viewer_control = ViewerControl()
        self.viewer_control.register_dragbox_cb(on_dragbox_created)

        def foobar(button: ViewerButton):
            def baz(click: ViewerDragbox):
                # self.eval()
                # if self.training:
                #     print("can't do anything while training...")
                #     self.button.set_disabled(False)
                #     self.viewer_control.unregister_pointer_cb(baz)
                #     return
                # print("foo", click)
                self.train()
                ...
                # self.cull_gaussians(extra_cull_mask=(~mask))
                self.means = Parameter(self.means[mask])
                self.scales = Parameter(self.scales[mask])
                self.quats = Parameter(self.quats[mask])
                self.features_dc = Parameter(self.features_dc[mask])
                self.features_rest = Parameter(self.features_rest[mask])
                self.opacities = Parameter(self.opacities[mask])
                ...
                if self.vis_counts is not None:
                    self.vis_counts = self.vis_counts[mask]
                    self.xys_grad_norm = self.xys_grad_norm[mask]
                    self.max_2Dsize = self.max_2Dsize[mask]

                self.remove_from_all_optim(self.optimizers, ~mask)

                self.button.set_disabled(False)
                self.viewer_control.unregister_pointer_cb(baz)

            self.button.set_disabled(True)
            self.viewer_control.register_dragbox_cb(baz)
        self.button = ViewerButton("remove gaussians", cb_hook=foobar)
